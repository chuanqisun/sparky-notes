...
export async function exportClaimsV2(outputDir: string) {
  await mkdir(outputDir, { recursive: true });
  await exportClaimByTypeV2(EntityType.Insight, outputDir);
}

export async function exportClaimByTypeV2(entityType: number, path: string) {
  const claimIndexProxy = getClaimIndexProxy(process.env.HITS_UAT_SEARCH_API_KEY!);

  const filter = ifAll([field("ClaimType").eq(entityType), field("Methods").any((method) => ifAny([method().eq("usability testing")]))]).toString();
  console.log(filter);
  ...
  await Promise.allSettled(
    ...
    .then(() => {
      return writeFile(`${path}/${EntityName[entityType]}.json`, JSON.stringify(results, null, 2));
    })
