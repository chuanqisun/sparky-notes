...
export function getModelType(deployment: ModelDeployment): "chat" | "embedding" {
  if (isChatModel(deployment)) {
    return "chat";
  } else if (isEmbeddingModel(deployment)) {
    return "embedding";
  } else {
    throw new Error(`Unknown model type for deployment ${deployment.id}`);
  }
}
...
export function isEmbeddingModel(deployment: ModelDeployment): boolean {
  return ["text-embedding-ada-002"].includes(deployment.model);
}
