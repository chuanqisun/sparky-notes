...
import { graphNodeToFtsDocument, searchResultDocumentToGraphNode, searchResultToGraphOperations } from "./modules/hits/adaptor";
...
const handleFullSync: WorkerRoutes["fullSync"] = async ({ req, emit }) => {
  ...
  const summary = await search({
    ...
    onProgress: async (progress) => {
      ...
      const graphOps = searchResultToGraphOperations(progress.items.map((item) => item.document));
      await dispatchGraphOps(await db, graphOps);
    },
  });
