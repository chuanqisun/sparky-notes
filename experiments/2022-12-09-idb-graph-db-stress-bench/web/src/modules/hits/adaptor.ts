...
import type { GraphOperation } from "../graph/graph";
...
export function searchResultToGraphOperations(searchResult: SearchResultDocument[]): GraphOperation[] {
  ...
  return searchResult.flatMap((document) => {
    const reportNode: HitsReportNode = {
      type: "report",
    ...
    const claimNodes: HitsClaimNode[] = qualifiedChildren.map((claim) => ({
      type: "claim",
    ...
    const outEdges: (HitsIncludesEdge | HitsTranscludesEdge)[] = qualifiedChildren.map((child, i) => ({
      type: child.isNative ? "includes" : "transcludes",
      from: document.id,
      to: child.id,
      updatedOn: reportNode.updatedOn > claimNodes[i].updatedOn ? reportNode.updatedOn : claimNodes[i].updatedOn, // the greater of parent and child
    }));

    return [
      {
        upsertNode: reportNode,
      },
