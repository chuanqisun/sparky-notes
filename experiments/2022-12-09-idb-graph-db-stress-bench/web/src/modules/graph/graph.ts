...
import type { EdgeSchema, GraphDB, NodeSchema, NodeSchemaV2 } from "./db";
import { tx } from "./tx";
...
export interface GraphOperation {
  upsertNode?: NodeSchemaV2;
  setOutEdges?: EdgeSchema[];
}

export const dispatchGraphOps = (db: GraphDB, ops: GraphOperation[]) =>
  tx(db, ["nodeV2", "edge"], "readwrite", async (tx) => {
    const nodeStore = tx.objectStore("nodeV2");
    const edgeStore = tx.objectStore("edge");

    for (let op of ops) {
      if (op.upsertNode) {
        nodeStore.put(op.upsertNode);
      }

      if (op.setOutEdges) {
        const fromIds = op.setOutEdges?.map((edge) => edge.from) ?? [];
        for (let fromId of fromIds) {
          const edgeToDelete = await edgeStore.index("byFromId").get(fromId);
          if (edgeToDelete) {
            edgeStore.delete(edgeToDelete.id!);
          }
        }

        for (let edge of op.setOutEdges) {
          edgeStore.put(edge);
        }
      }
    }
